...
| 2026-08-31 | **Ported catalog issue #230 from the sibling pattern.** This scaffold carried the same three copies of the #185 preamble strip, and the same two of them were missing every backslash — `prd-phase.mjs` and `publish-tickets.mjs` stripped nothing, so a ticket written literally from the shipped `ticket.template.md` was rejected as `no-frontmatter` by the publisher and never became a tracker issue. `stripPreamble` is now exported from `dag-core.mjs` and imported by both, identical to the sibling apart from this pattern's runtime path in the publisher's usage line. | Ported from the sibling pattern, 2026-08-31 (catalog issue #230, reported by adopter `fx-eye-tracking`); verified by `suite-codex-three` 160/160. | Horace Hou (direction) / Claude Opus 5 (build) |
| 2026-08-31 | **Ported catalog issue #233 from the sibling pattern.** `tree-fingerprint.mjs` added and `deliver-ticket.mjs` gained `--expect-tree`, so a working tree that changed after the build refuses the merge instead of being attested to. Unlike the #230 port, the producer side is wired here too: `builder.toml` now runs the fingerprint as its last action and returns `treeSha`. This pattern has no write guard of its own (its sandbox handles that), so the blocklist half of #233 does not apply — but the fingerprint is the half that does not depend on a blocklist, which is the half worth having. | Ported from the sibling pattern, 2026-08-31 (catalog issue #233); verified by `suite-codex-three` 163/163. | Horace Hou (direction) / Claude Opus 5 (build) |
