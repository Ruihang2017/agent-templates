...
| **The real executor has never been invoked by this scaffold.** The E2E suite drives real git, real worktrees and real merges, but a stand-in executor stands in for `codex exec`. So the *driver's* behaviour is tested; the *integration* is not. Specifically unverified: that the real binary writes a schema-conforming JSON object to the `-o` path in the shape the driver parses, that a `low`-effort executor produces usable work from a brief, and that `spawn('codex')` resolves on Windows without a shell | Design/verification gap as of 2026-08-10. The flag set itself **is** verified against `codex-cli 0.147.0` (§6) — what is untested is the runtime behaviour behind those flags | A Level-1 rehearsal against the real binary, on a throwaway repo with one trivial brief, closes most of this in a single run. Until then, treat the executor boundary as the least-proven part of the pattern | 2026-08-10 |
