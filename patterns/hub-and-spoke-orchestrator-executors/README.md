...
| **The hub side has still never been run.** `/hub-brief`, `/hub-dispatch` and `/hub-collect` have not been executed by a real Opus session. The 2026-08-10 rehearsal drove the scripts directly with a **hand-written** brief, so brief *quality* from the real command — the thing everything downstream depends on — remains unverified | Verification gap as of 2026-08-10 | Only a real project run closes this, which is also what promotion to `adopted` requires. Until then the largest risk (§4 row 2: wrong decomposition is unrecoverable downstream) has never been exercised against a brief this pattern actually produced | 2026-08-10 |
