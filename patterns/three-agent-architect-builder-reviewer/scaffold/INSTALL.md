...
- **Wave mode (the target operating model):** `/start-milestone <module> [mode]` or `/start-all [mode] [concurrency]` — Gate 1 in one action: verify inputs, publish tickets as issues, then loop `wave-plan.mjs` → `Workflow(run-wave)` → compose PR bodies → `deliver-wave.mjs` until the graph is exhausted. Each stage still runs in its own subagent; ordering, the bounce cap, the acceptance guard and the wave-independence check are code, not prompts, and so are scheduling and delivery policy. The human returns at Gate 2 (smoke test) or on escalation.
- **Delivery runs in YOUR session, not a subagent** (upstream issue #206). A workflow script has no filesystem and no exec, so nothing inside a run can invoke the delivery script except an agent — and that agent made no judgement while its separate context forced the Reviewer's approval to be re-typed by a third party, which safety classifiers blocked. Consequences worth knowing before your first run: a workflow run **merges nothing**, tickets move in waves (a blocker must land before its dependent starts), and the main-session write guard allows `.claude/tmp/` so you can compose the PR body.
- **Single ticket, by hand:** `/deliver-ticket <ticket-id> [supervised]` after a CLEAR verdict. This is the manual path the stage commands stop short of — `/review-ticket` ends at the verdict.
