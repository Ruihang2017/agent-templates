...
- **Delivering a single ticket by hand:** `/deliver-ticket <ticket-id> [supervised]` after a CLEAR verdict. `/review-ticket` ends at the verdict, so without this there is no sanctioned delivery step outside a pipeline run and an operator ends up invoking the script themselves. It runs in the main session; the write guard allows `.claude/tmp/` for the PR body it composes, and nothing else.
