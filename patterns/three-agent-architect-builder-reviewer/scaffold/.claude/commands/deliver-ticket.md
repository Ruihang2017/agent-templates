...
5. **Delete the branch only after it landed.** `git branch -D ticket/<id>` once `outcome: "delivered"` and `dodPassed: true`. A leftover branch for a ticket that did NOT land is evidence and stays. (A wave run does this for you via `deliver-wave.mjs`.)
