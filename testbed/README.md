...
| `suite-wave` | The ACTUAL `run-wave.js`, executed with stubbed `agent()`: happy path, bounce-then-clear, bounce-cap escalation, reviewer-failure handling, the acceptance guard, prompt isolation, worktree isolation, concurrency capping — plus the two guarantees the wave design exists to protect: the workflow spawns **no delivery agent and merges nothing**, and the Reviewer **authors its own record** (no verdict text crosses a context boundary). |
| `suite-waveplan` | `wave-plan.mjs`, the deterministic scheduler: a wave never contains an internal dependency; delivering one wave unlocks the next; a failed blocker makes its dependents `unreachable` rather than silently dropping them; cycles and dangling `blocked_by` exit non-zero; module scope honours cross-module blockers; the local ledger at `docs/delivered.json` is the resume signal with no tracker; an empty wave that is not `done` never reads as completion. |
| `suite-deliverwave` | `deliver-wave.mjs`, the delivery policy: which flags each mode implies (tracker / local / supervised / integration-branch), an unevidenced review is **refused** rather than delivered, non-CLEAR rows are reported not dropped, nothing that failed to land is counted or has its branch deleted, and delivery is sequential. `deliver-ticket.mjs` itself is stubbed via `DELIVER_TICKET_BIN` — one case deliberately runs with that override **removed**, because every other case using it is how a broken default path once went unnoticed. |
