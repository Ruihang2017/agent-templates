...
1. `testbed/rehearsal/src/unique.mjs` — export `unique` as a **named ESM export**.
2. `testbed/rehearsal/tests/unique.test.mjs` — tests using `node:test` and `node:assert/strict`, importing from `../src/unique.mjs`.

Behaviour: Return a new array with duplicates removed, preserving first-seen order.

## File scope

- **Touches:** `testbed/rehearsal/src/unique.mjs`, `testbed/rehearsal/tests/unique.test.mjs`
