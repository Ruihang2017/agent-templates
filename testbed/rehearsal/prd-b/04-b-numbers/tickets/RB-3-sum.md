...
1. `testbed/rehearsal/src/sum.mjs` — export `sum` as a **named ESM export**.
2. `testbed/rehearsal/tests/sum.test.mjs` — tests using `node:test` and `node:assert/strict`, importing from `../src/sum.mjs`.

Behaviour: Return the arithmetic sum of an array of numbers; `sum([])` is 0. Throw a `TypeError` when any element is not a finite number.

## File scope

- **Touches:** `testbed/rehearsal/src/sum.mjs`, `testbed/rehearsal/tests/sum.test.mjs`
