...
1. `testbed/rehearsal/src/clamp.mjs` — export `clamp` as a **named ESM export**.
2. `testbed/rehearsal/tests/clamp.test.mjs` — tests using `node:test` and `node:assert/strict`, importing from `../src/clamp.mjs`.

Behaviour: Return `n` bounded to the inclusive range [lo, hi]. Throw a `RangeError` when `lo > hi`.

## File scope

- **Touches:** `testbed/rehearsal/src/clamp.mjs`, `testbed/rehearsal/tests/clamp.test.mjs`
