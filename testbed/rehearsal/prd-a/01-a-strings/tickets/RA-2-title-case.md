...
1. `testbed/rehearsal/src/title-case.mjs` — export `titleCase` as a **named ESM export**.
2. `testbed/rehearsal/tests/title-case.test.mjs` — tests using `node:test` and `node:assert/strict`, importing from `../src/title-case.mjs`.

Behaviour: Uppercase the first letter of every whitespace-separated word; leave the rest of each word untouched.

## File scope

- **Touches:** `testbed/rehearsal/src/title-case.mjs`, `testbed/rehearsal/tests/title-case.test.mjs`
