...
1. `testbed/rehearsal/src/slugify.mjs` — export `slugify` as a **named ESM export**.
2. `testbed/rehearsal/tests/slugify.test.mjs` — tests using `node:test` and `node:assert/strict`, importing from `../src/slugify.mjs`.

Behaviour: Lowercase the input, replace every run of non-alphanumeric characters with a single `-`, and trim leading/trailing `-`.

## File scope

- **Touches:** `testbed/rehearsal/src/slugify.mjs`, `testbed/rehearsal/tests/slugify.test.mjs`
