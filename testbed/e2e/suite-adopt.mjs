...
export async function run() {
  ...
  try {
    ...
    // Retired files (catalog issue #208). Re-adopting copies new files in, but nothing told
    // it that an OLD file had become wrong — so a file the pattern has since withdrawn would
    // sit in the target beside its replacement, still loadable, still carrying the behaviour
    // the new version exists to remove. Simulated by planting one and re-adopting.
    {
      const stale = join(t1, '.claude', 'workflows', 'run-wave.js')
      mkdirSync(dirname(stale), { recursive: true })
      writeFileSync(stale, '// a scheduler from a withdrawn version\n')
      ...
      check(S, 'A1r and its removal is reported with a reason',
        again.stdout.includes('- retire  .claude/workflows/run-wave.js') && /issue #208/.test(again.stdout))
      check(S, 'A1r the current scheduler is still installed',
        existsSync(join(t1, '.claude', 'workflows', 'run-milestone.js')))
