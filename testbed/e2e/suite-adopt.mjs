...
export async function run() {
  ...
  try {
    ...
    for (const f of ['.claude/workflows/run-wave.js', '.claude/workflows/nightly-issues.js', '.claude/scripts/wave-plan.mjs', '.claude/scripts/deliver-wave.mjs', '.claude/hooks/guard-main-session-writes.mjs', '.claude/scripts/publish-tickets.mjs', '.claude/scripts/deliver-ticket.mjs', '.claude/scripts/asana-sync.mjs']) {
    ...
    // issue #153: dag-report.mjs rewrites docs/prd/dag.html on every run, so without an
    // eol rule a Windows checkout keeps it permanently modified with an EMPTY diff and
    // deliver-ticket refuses to merge — one generated file blocking every delivery.
    check(S, 'A1 .gitattributes pins the generated dag.html to LF', ga1.includes('docs/prd/dag.html text eol=lf'))
    // it must carry its OWN marker: an install predating this rule already has the
    // workflow-scripts marker, so a shared one would skip it forever
    check(S, 'A1 the dag.html rule has its own marker', /generated by dag-report\.mjs/.test(ga1))
    check(S, 'A1 CLAUDE.md declares Operating mode', /Operating mode/.test(readFileSync(join(t1, 'CLAUDE.md'), 'utf8')))

    // Retired files (catalog issue #206). Re-adopting copies new files in, but nothing
    // told it that an OLD file had become wrong — so a retired scheduler would sit in the
    // target beside its replacement, still loadable, still carrying the behaviour the new
    // version exists to remove. Simulated here by planting the retired file and re-adopting.
    {
      const stale = join(t1, '.claude', 'workflows', 'run-milestone.js')
      mkdirSync(dirname(stale), { recursive: true })
      writeFileSync(stale, '// a scheduler from an earlier version\n')
      const again = runAdopt([PATTERN, t1, '--platform', 'gh'])
      eq(S, 'A1r re-adopt exits 0', again.status, 0)
      check(S, 'A1r a retired file is REMOVED on re-adopt', !existsSync(stale))
      check(S, 'A1r and its removal is reported with a reason',
        again.stdout.includes('- retire  .claude/workflows/run-milestone.js') && /run-wave/.test(again.stdout))
      check(S, 'A1r the replacement is still installed', existsSync(join(t1, '.claude', 'workflows', 'run-wave.js')))
      check(S, 'A1r the user is told to review the deletion before committing', /review it before committing/.test(again.stdout))
    }
    ...
    eq(S, 'A2 the dag.html eol rule is present exactly once', (readFileSync(join(t1, '.gitattributes'), 'utf8').match(/docs\/prd\/dag\.html text eol=lf/g) || []).length, 1)
