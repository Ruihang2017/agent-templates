...
2. **Self-hosted pipeline.** This repo runs the pattern on itself. Two uses: the nightly `/nightly-issues` sweep (other projects file pattern-tweak requests here via `.github/ISSUE_TEMPLATE/`, and the sweep triages — and where fixable, fixes — them overnight), and **Level-1 rehearsals** that exercise the full `/start-all` pipeline against `testbed/rehearsal/` (issue #75). The machinery at `.claude/` — agents, workflows, commands, and the deterministic `scripts/` — is a **byte-synced copy of the scaffold**, enforced by the E2E integrity suite: change the scaffold first, then re-copy. Only `settings.json` is catalog-specific (see item 4).
   **A rehearsal never writes `main`.** `defaultBranch` flows from the workflow args through to `deliver-ticket.mjs --default-branch`, so a rehearsal run is pointed at a sacrificial branch and every ticket branch, PR, and merge lands there. Its tracker issues are real — `dodPassed` gates on `issueClosed`, so a run without a tracker cannot satisfy the Definition of Done — and they are the durable evidence record once the branch is deleted.
