...
async function runTicket(t, opts) {
  ...
  const deliverCmd = 'node .claude/scripts/deliver-ticket.mjs --id ' + t.id + ' --branch ' + branch +
    ' --default-branch ' + cfg.defaultBranch + ((build && build.headSha) ? ' --expect-head ' + String(build.headSha).trim() : '') + ((build && build.treeSha) ? ' --expect-tree ' + String(build.treeSha).trim() : '') + (LOCAL_ONLY ? ' --delivery local' : ' --platform ' + cfg.platform + (t.issue ? ' --issue ' + t.issue : '')) +
