...
// pushing. A scratch file under .claude/tmp/ would be lost on the first clean checkout —
...
// A ledger under `.claude/` or `.codex/` would give each runtime its OWN record of what
...
const writeLedger = () => {
  ...
  try {
    // A repo may reach `local` delivery without a `.claude/` directory — the mode is
