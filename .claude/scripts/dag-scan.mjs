...
//
// It is also what makes the DAG dynamic. Workflow scripts cannot touch the filesystem
// (no `require`, no `node:` imports — suite-integrity enforces it), so a running
// /start-all learns about a ticket added mid-run only by having an agent execute this
// script and hand back the JSON. Keep the output shape stable; the scheduler diffs it.
//
// Usage: node .claude/scripts/dag-scan.mjs [prd-root]     (default: docs/prd)
